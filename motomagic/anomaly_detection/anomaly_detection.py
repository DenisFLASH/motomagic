...
def calc_anomaly_scores(X, block_ids):
    ...
    block_scores = dict()
    for pred, name in zip(scores_pred, block_ids):
        block_scores[name] = pred
    sorted_scores = sorted(block_scores.items(), key=lambda x: x[1])
